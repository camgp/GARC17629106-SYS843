...
class UnetSpectrogramSeparator:
    ...
    def __init__(self, model_config):
        ...
        assert (
                    len(self.source_names) == 2)  # Only use for acc/voice separation for now, since model gets too big otherwise
    ...
    def get_output(self, input, training, return_spectrogram=False, reuse=True):
        ...
        with tf.variable_scope("separator", reuse=reuse):
            ...
            stfts = tf.contrib.signal.stft(tf.squeeze(input, 2), frame_length=self.frame_len, frame_step=self.hop,
                                           fft_length=self.frame_len, window_fn=window)
            ...
            mix_mag_norm = mix_mag_norm[:, :, :-1,
                           :]  # Cut off last frequency bin to make number of frequency bins divisible by 2

            mags = dict()
            for name in self.source_names:
                current_layer = mix_mag_norm
                current_layer = tf.layers.conv2d(current_layer, 128, [3, 3], strides=[2, 2], activation=None,
                                                 padding='same')
                current_layer = tf.contrib.layers.batch_norm(current_layer, activation_fn=LeakyReLU,
                ...
                for i in range(self.num_layers):
                    assert (current_layer.get_shape().as_list()[1] % 2 == 0 and current_layer.get_shape().as_list()[
                        2] % 2 == 0)
                    # block
                ...
                # Compute mask
                mask = tf.layers.conv2d_transpose(current_layer, 1, [3, 3], strides=[2, 2], activation=tf.nn.sigmoid,
                                                  padding="same")
                mask = tf.pad(mask, [(0, 0), (0, 0), (0, 1), (0, 0)], mode="CONSTANT",
                              constant_values=0.5)  # Pad last frequency bin of mask that is missing since we removed it in the input
            ...
            if return_spectrogram:
                ...
                for source_name in mags.keys():
                    ...
                    audio = tf.contrib.signal.inverse_stft(stft, self.frame_len, self.hop, self.frame_len,
