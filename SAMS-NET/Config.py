...
def cfg():
    # Base configuration
    model_config = {"musdb_path": "C:/Users/camil/PycharmProjects/SYS843-music_separation/MUSDB18/",
                    # SET MUSDB PATH HERE
                    "estimates_path": "C:/Users/camil/PycharmProjects/SYS843-music_separation/SAMS-NET/Source_Estimate/",
                    # SET THIS PATH TO WHERE YOU WANT SOURCE ESTIMATES PRODUCED BY THE TRAINED MODEL TO BE SAVED. Folder itself must exist!
                    "data_path": "C:/Users/camil/PycharmProjects/SYS843-music_separation/SAMS-NET/data/",
                    ...
                    'augmentation': True,
                    # Random attenuation of source signals to improve generalisation performance (data augmentation)
                    'raw_audio_loss': True,
    ...
    if model_config["task"] == "multi_instrument":
        model_config["source_names"] = ["bass_snet", "drums_snet", "other_snet", "vocals_snet"]
    elif model_config["task"] == "voice":
        model_config["source_names"] = ["accompaniment_snet", "vocals_snet"]
    else:
...
def baseline_stereo():
    print(
        "Training baseline model with difference output and input context (valid convolutions) and stereo input/output")
    model_config = {
        "output_type": "difference",
        "context": True,
        "mono_downmix": False
...
def full():
    print(
        "Training full singing voice separation model, with difference output and input context (valid convolutions) and stereo input/output, and learned upsampling layer")
    model_config = {
        "output_type": "difference",
        "context": True,
        "upsampling": "learned",
...
def baseline_comparison():
    model_config = {
        "batch_size": 4,
        # Less output since model is so big. Doesn't matter since the model's output is not dependent on its output or input size (only convolutions)
...
def unet_spectrogram():
    model_config = {
        "batch_size": 2,  # Less output since model is so big.
