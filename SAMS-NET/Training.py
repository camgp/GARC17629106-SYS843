...
import Datasets
import Utils
import UnetSpectrogramSeparator
...
def train(model_config, experiment_id, load_model=None):
    ...
    if model_config["network"] == "unet":
        separator_class = UnetAudioSeparator.UnetAudioSeparator(model_config)
    elif model_config["network"] == "unet_spectrogram":
        separator_class = UnetSpectrogramSeparator.UnetSpectrogramSeparator(model_config)
    else:
    ...
    # BUILD MODELS
    # Separator
    separator_sources = separator_func(batch["mix"], True, not model_config["raw_audio_loss"],
                                       reuse=False)  # Sources are output in order [acc, voice] for voice separation, [bass, drums, other, vocals] for multi-instrument separation
    ...
    # define NumVars
    global num_var
    num_var = str(Utils.getNumParams(separator_vars))

    update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS)
    with tf.control_dependencies(update_ops):
        with tf.variable_scope("separator_solver"):
            separator_solver = tf.train.AdamOptimizer(learning_rate=model_config["init_sup_sep_lr"]).minimize(
    ...
    if best:
        save_path = saver.save(sess,
                               model_config["model_base_dir"] + os.path.sep + str(experiment_id) + os.path.sep + str(
                                   experiment_id), global_step=int(0))
    else:
        save_path = saver.save(sess,
                               model_config["model_base_dir"] + os.path.sep + str(experiment_id) + os.path.sep + str(
                                   experiment_id), global_step=int(1))
...
def optimise(model_config, experiment_id):
    ...
    for i in range(2):
        ...
        while worse_epochs < model_config["worse_epochs"]:  # Early stopping on validation set after a few epochs
            ...
            curr_loss = Test.test(model_config, model_folder=str(experiment_id), partition="valid",
            ...
            f = open(loss_path, 'a')
            if epoch == 1:
                f.write('NumVars: ' + num_var + '\n')
            f.write(model_path + ' ' + 'EPOCH: ' + str(epoch) + ' ' + str(curr_loss) + '\n')
            f.close()
...
def run(cfg):
    ...
    # sup_model_path = 'checkpoints/999079/999079-48000'
